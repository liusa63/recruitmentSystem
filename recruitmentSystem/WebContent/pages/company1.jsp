			 ...
			 var reg=/^\d+$/;
			  if(reg.test(gopage)){
				  if(gopage>0 && gopage<=countPage){
					  window.location.href="${pageContext.request.contextPath}/CompanyListServlet?uid="+<%=uid%>+"&currentPage="+gopage;
				  }else{
...
<div class="jobs-table">
				...
				<c:forEach items="${companyPage.list}" var="s">
				<div class="table-row">
					<div class="table-cells flex no-wrap">
						...
						<div class="cell salary-cell flex no-column no-wrap">
							...
							<a href="ComDeliveryResumeServlet?uid=<%=uid%>&pid=${s.pid}&dstatus=1">投递简历</a>
						</c:if>
						<%} %></p>
						</div>
					</div> <!-- end .table-cells -->
               ...
               <div class="table-footer flex space-between items-center">
					...
					<div class="jobpress-custom-pager list-unstyled flex space-between no-column items-center">
						<a href="${pageContext.request.contextPath}/CompanyListServlet?currentPage=1&uid=<%=uid%>" class="button">首页</a>
						<c:if test="${companyPage.currentPage>1}">
						<a href="${pageContext.request.contextPath}/CompanyListServlet?currentPage=${companyPage.currentPage-1}&uid=<%=uid%>" class="button"><i class="ion-ios-arrow-left"></i>Prev</a>
						</c:if>
						...
						<a href="${pageContext.request.contextPath}/CompanyListServlet?currentPage=${companyPage.currentPage+1}&uid=<%=uid%>" class="button">Next<i class="ion-ios-arrow-right"></i></a>
					    </c:if>
					    <a href="${pageContext.request.contextPath}/CompanyListServlet?currentPage=${companyPage.countPage}&uid=<%=uid%>" class="button">尾页</a>
					    ...
					    <a  onclick="jumpfun();" class="button" >跳转</a>
